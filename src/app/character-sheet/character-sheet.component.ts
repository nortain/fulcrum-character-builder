...
import {CharacterModel} from "../shared/character/character-model";
...
export class CharacterSheetComponent implements OnInit, OnChanges {
  ...
  constructor(private characterService: CharacterFactoryService, private attributeService: AttributeService, private modalService: NgbModal) {
    this.testName = "";
  }
  ...
  ngOnInit() {
    ...
    this.character = this.characterService.getNewCharacter("", RaceType[raceType] as RaceType);
  }

  reloadCharacter(propertyName: string, valueChange: any) {
    console.log("CharacterFactoryService has been reloaded");
  ...
  startReloadWithRace(raceString: RaceType) {
    if (RaceType[raceString] !== RaceType.Primental) {
      this.character.race.racialSubType = null;
    }
  ...
  getHitPointsValue(): number {
    return this.characterService.getHitPoints(this.character);
  }
